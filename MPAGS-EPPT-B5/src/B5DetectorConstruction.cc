...
G4VPhysicalVolume* B5DetectorConstruction::Construct()
{
  ...
  // add a 5cm block of lead immidiately before the magnetic boxs as seem along particle path
  // remember the half lengths!!!!
  auto leadPlateSolid = new G4Box("PbPlate", 1.*m, 1.*m, 0.5*cm);
  auto leadPlateLogical = new G4LogicalVolume(leadPlateSolid,lead,"leadPlateLogical");
  G4double z_PbPlate = -1.2*m; //define the placement of the leaad plate relative to the world centre
  new G4PVPlacement(0,G4ThreeVector(0.,0.,z_PbPlate),leadPlateLogical, "leadPlatePhysical",worldLogical,
                    false,0,checkOverlaps);
  auto  leadPlateVisAttributes = new G4VisAttributes(G4Colour(0.0, 0.0, 0.9));
  leadPlateLogical->SetVisAttributes(leadPlateVisAttributes);
